...
class ResourceManagerDa extends UserDa{
    ...
    findUsersBySkill(skillIds, filters, skip, limit){
        ...
        let match = `match (n:${label})-[:${officeRelL}]->(o) where not(n.disabled) ${whereOffices}
                     ...
                     optional match (n)-[k:${kRelL}]->(s:${skillL})-[:${groupLbl}]->(sg) where sg.type in ["tool", "skill"] and id(s) in {skillIds} and not(k.want)
        ...
        let cmd = `${match}
                    ...
                    with n, o, p, al, collect(distinct a) as approvers, 
                        filter(x IN collect(distinct {_:s, level: k.level, approved: k.approved}) where x._ is not null) as skills,
                    ...
                    ( reduce(acc = 0, s IN skills | acc + s.level) + size(industries)*2 + size(interests) ) as score
